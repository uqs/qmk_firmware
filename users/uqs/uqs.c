...
// July 2021, 68wpm/98% on MT; 69wpm/98% on typeracer
...
const rgblight_layer_t PROGMEM my_rgb_segments[] = {
    [L_QWER] = {{0, RGBLIGHT_LED_COUNT, HSV_WHITE},  RGBLIGHT_END_SEGMENTS},
    [L_WASD] = {{0, RGBLIGHT_LED_COUNT/2, HSV_RED}, {RGBLIGHT_LED_COUNT/2, RGBLIGHT_LED_COUNT/2, HSV_OFF}, RGBLIGHT_END_SEGMENTS},
    [L_COLM] = {{0, RGBLIGHT_LED_COUNT, HSV_GREEN},  RGBLIGHT_END_SEGMENTS},
    [L_EXTD] = {{0, RGBLIGHT_LED_COUNT, HSV_BLUE},   RGBLIGHT_END_SEGMENTS},
    [L_NUM] =  {{0, RGBLIGHT_LED_COUNT, HSV_ORANGE}, RGBLIGHT_END_SEGMENTS},
    [L_FUNC] = {{0, RGBLIGHT_LED_COUNT, HSV_YELLOW}, RGBLIGHT_END_SEGMENTS},
    [L_MOUSE]= {{0, RGBLIGHT_LED_COUNT, HSV_PURPLE}, RGBLIGHT_END_SEGMENTS},
};
...
#if 0
    // From drashna, but it a) kicks in with every move, even when the keyboard
    // only shakes a little, and it b) kills a manual layer change, and c)
    // needs to not turn off when a mouse button is held for dragging. I don't
    // fully understand how that's done in the code and frankly haven't warmed
    // to the auto-layer idea yet.
    int8_t x = mouse_report.x, y = mouse_report.y;
    if (x != 0 || y != 0) {
        mouse_timer = timer_read();
        if (!layer_state_is(L_MOUSE)) {
            dprintf("turning on mouse layer\n");
            layer_on(L_MOUSE);
        }
    } else if (timer_elapsed(mouse_timer) > 450 && layer_state_is(L_MOUSE) && !mouse_keycode_tracker) {
        // XXX don't turn off if the layer key is manually held! is that what
        // the tracker tries to do?
        layer_off(L_MOUSE);
